...
export type SitePageFieldsEnum =
  ...
  | 'pluginCreator___pluginOptions____x'
  | 'pluginCreator___pluginOptions____xcomponents'
...
export type Unnamed_1_Query = { toppings: { nodes: Array<Pick<SanityTopping, 'id' | 'name' | 'vegetarian'>> }, pizzas: { nodes: Array<(
      Pick<SanityPizza, 'id' | 'name'>
      & { toppings?: Maybe<Array<Maybe<Pick<SanityTopping, 'id' | 'name'>>>> }
    )> } };

export type AllSanityPizzaQueryVariables = Exact<{ [key: string]: never; }>;


export type AllSanityPizzaQuery = { allSanityPizza: { nodes: Array<TestFragmentFragment> } };

export type TestFragmentFragment = (
  Pick<SanityPizza, 'id' | 'name'>
  & { slug?: Maybe<Pick<SanitySlug, 'current'>>, toppings?: Maybe<Array<Maybe<Pick<SanityTopping, 'id' | 'name'>>>>, image?: Maybe<{ asset?: Maybe<{ fixed?: Maybe<GatsbySanityImageFixedFragment>, fluid?: Maybe<GatsbySanityImageFluidFragment> }> }> }
);
